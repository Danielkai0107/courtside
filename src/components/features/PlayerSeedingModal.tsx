...
const PlayerSeedingModal: React.FC<PlayerSeedingModalProps> = ({
  ...
  const [formatType, setFormatType] = useState<"knockout" | "group" | "round_robin">("knockout");
  ...
  useEffect(() => {
    ...
    // 判斷賽制類型
    const hasGroupStage = selectedFormat.stages.some((s) => s.type === "group_stage");
    const hasRoundRobin = selectedFormat.stages.some((s) => s.type === "round_robin");
    ...
    if (hasRoundRobin) {
      ...
      setFormatType("group");
      // 小組賽：顯示分組
      const groupStage = selectedFormat.stages.find((s) => s.type === "group_stage");
      ...
      // 淘汰賽：顯示第一輪配對
      const pairs: Array<[number, number]> = [];
      const bracketSize = Math.pow(2, Math.ceil(Math.log2(orderedParticipants.length)));
          ...
          <div key={pairIndex} className={styles.matchup}>
            ...
            <div className={styles.vsContainer}>
              <div className={styles.playerSelect}>
                ...
                <select
                  value={index1}
                  onChange={(e) => handleSwap(index1, parseInt(e.target.value))}
              ...
              <div className={styles.playerSelect}>
                ...
                <select
                  value={index2}
                  onChange={(e) => handleSwap(index2, parseInt(e.target.value))}
                  className={styles.dropdown}
                >
                  {orderedParticipants.map((p, idx) => (
                    <option key={idx} value={idx}>
          ...
          <div className={styles.matchup}>
            ...
            <div className={styles.byePlayer}>
              ...
              <select
                ...
                onChange={(e) => handleSwap(orderedParticipants.length - 1, parseInt(e.target.value))}
                className={styles.dropdown}
              >
                {orderedParticipants.map((p, idx) => (
                  <option key={idx} value={idx}>
            ...
            <div className={styles.groupMembers}>
              {group.map((playerIndex) => (
                <div key={playerIndex} className={styles.groupMember}>
                  ...
                  <select
                    value={playerIndex}
                    onChange={(e) => handleSwap(playerIndex, parseInt(e.target.value))}
                    className={styles.dropdown}
                  >
                    {orderedParticipants.map((p, idx) => (
                      <option key={idx} value={idx}>
      ...
      <div className={styles.seedList}>
        {orderedParticipants.map((participant, index) => (
          <div key={index} className={styles.seedItem}>
            ...
            <select
              value={index}
              onChange={(e) => handleSwap(index, parseInt(e.target.value))}
              className={styles.dropdown}
            >
              {orderedParticipants.map((p, idx) => (
                <option key={idx} value={idx}>
    ...
    >
      <div className={styles.container}>
        <div className={styles.description}>
          <p>
            調整選手的種子順序，以改變對戰配對。選擇選手後會自動交換位置。
          </p>
          <p className={styles.note}>
            💡 提示：這不會影響晉級流向和積分算法，僅調整初始配對。
          </p>
        </div>
