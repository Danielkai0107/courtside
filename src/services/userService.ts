...
export const linkShadowAccounts = async (user: User) => {
  ...
  try {
    console.log(
      `🔗 [linkShadowAccounts] 開始同步影子帳號和邀請: ${user.email}`
    );
    ...
    if (!playersSnapshot.empty) {
      ...
      await batch.commit();
      console.log(
        ` [linkShadowAccounts] 連結了 ${playersSnapshot.size} 個選手影子帳號`
      );
    }
    ...
    if (!staffSnapshot.empty) {
      console.log(
        `📧 [linkShadowAccounts] 找到 ${staffSnapshot.size} 個待處理的紀錄員邀請`
      );
        ...
        if (!alreadyNotified) {
          ...
          try {
            ...
            if (tournament) {
              ...
              console.log(
                ` [linkShadowAccounts] 為邀請創建通知: ${tournament.name}`
              );
            }
          } catch (notifError) {
          ...
          console.log(
            `⏭️ [linkShadowAccounts] 跳過已通知的邀請: ${tournamentId}`
          );
        }
      }
      ...
      await batch.commit();
      console.log(
        ` [linkShadowAccounts] 更新了 ${staffSnapshot.size} 個紀錄員邀請`
      );
    }
  } catch (error) {
