...
- `src/types/index.ts` - 第 6 行：StageType 定義
- `src/types/index.ts` - 第 150 行：Match.stage 定義

### 實際使用

- `src/components/features/PlayerSeedingModal.tsx` - 第 38-52 行：類型映射
- `src/components/features/CategoryPublisher.tsx` - 第 95-97 行：檢查賽制
- `src/services/bracketService.ts` - 第 207 行：Match.stage 使用
- `src/services/scheduleRegenerationService.ts` - 第 107-109 行：檢查賽制
