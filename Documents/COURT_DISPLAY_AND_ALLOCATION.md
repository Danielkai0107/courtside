# 場地顯示與分配時機說明

## ✅ 修復的問題

### 1. 場地顯示名稱而非 ID ⭐

**修復前**：
```
小組賽：Court 01 顯示為 "abc123xyz"（courtId）
對陣圖：顯示 "abc123xyz"
計分頁面：顯示 "abc123xyz"
```

**修復後**：
```
小組賽：顯示 "Court 01"（court.name）✅
對陣圖：顯示 "Court 01"
計分頁面：顯示 "Court 01"
```

**實現方式**：
```typescript
// 載入場地列表
const courtsData = await getCourts(tournamentId);
setCourts(courtsData);

// Helper function
const getCourtName = (courtId) => {
  if (!courtId) return "待分配";
  const court = courts.find(c => c.id === courtId);
  return court?.name || courtId;
};

// 使用
{getCourtName(match.courtId)}
```

### 2. CategoryDetail 載入場地
- ✅ 載入場地列表
- ✅ 小組賽比賽顯示場地名稱
- ✅ 對陣圖顯示場地名稱

### 3. ScoringConsole 載入場地
- ✅ 訂閱比賽時同時載入場地
- ✅ 場地信息顯示名稱

---

## 🕐 場地分配時機說明

### 發布賽程時分配（第一輪）⭐

```
主辦方操作：
1. 場地管理 → 新增 Court 01, 02, 03
2. 賽程管理 → 選擇分類（男子雙打）
3. 選擇方案 → 點擊「發布賽程」
   ↓
系統自動執行：
4. 生成小組賽/淘汰賽比賽
5. 【立即分配場地】⭐
   - 小組賽：按小組固定場地
   - 淘汰賽第一輪：輪流分配
6. 寫入 Firestore

結果：
  小組賽：全部已分配場地 ✅
  第一輪：全部已分配場地 ✅
  第二輪：PENDING_PLAYER（等待晉級）
```

### 場地分配邏輯

#### 小組賽（發布時）
```typescript
// generateGroupThenKnockout() 中
const groupMatches = generateGroupStage(...);
assignCourtsToMatches(groupMatches, courts);  // ← 立即分配

結果：
  Group A 所有比賽 → Court 01
  Group B 所有比賽 → Court 02
  ...
  狀態：SCHEDULED（已排程）✅
```

#### 淘汰賽第一輪（發布時）
```typescript
// generateKnockoutOnly() 中
const firstRoundMatches = matches.filter(m => m.round === 1);
assignCourtsToMatches(firstRoundMatches, courts);  // ← 立即分配

結果：
  R16 Match 1 → Court 01
  R16 Match 2 → Court 02
  R16 Match 3 → Court 03
  R16 Match 4 → Court 01（循環）
  ...
  狀態：SCHEDULED（已排程）✅
```

#### 淘汰賽後續輪次（不分配）
```typescript
// generateKnockoutOnly() 中
for (let round = 2; round <= totalRounds; round++) {
  const match = createMatchNode({
    player1Id: null,  // 等待晉級
    player2Id: null,
    courtId: null,    // ← 不分配場地
    status: "PENDING_PLAYER",
  });
}

結果：
  QF, SF, FI：
    - courtId: null
    - 狀態：PENDING_PLAYER
    - 顯示：「待分配場地」
```

### 後續輪次什麼時候分配？⭐

**當前實現**：不自動分配

**原因**：
- 第二輪比賽要等第一輪完成
- 時間不確定
- 可能需要調整場地

**未來可以實現**：
1. 第一輪全部完成後，自動分配第二輪
2. 使用場地調度系統（dispatchCourtToWaitingMatch）
3. 或主辦方使用「重新分配場地」功能

**手動方式（當前）**：
```
主辦方在賽程管理 Tab
  ↓
選擇分類
  ↓
點擊「🔄 重新分配場地」
  ↓
系統重新分配所有未開始的比賽
  （包括第二輪）
```

---

## 📊 完整的場地分配流程

### 時間軸

```
T0: 發布賽程
  ├─ 小組賽：全部分配 ✅
  ├─ R16：全部分配 ✅
  ├─ QF：不分配（等待晉級）
  ├─ SF：不分配
  └─ FI：不分配

T1: 小組賽/R16 進行中
  → 場地狀態：IN_USE

T2: 小組賽/R16 完成
  → 選手晉級到 QF
  → QF 狀態：PENDING_COURT

T3: 主辦方重新分配（可選）⭐
  → QF：分配場地
  → 狀態：SCHEDULED
  
或

T3: 場地自動調度（未來實現）
  → 場地空閒時自動分配給 QF
```

---

## 📋 修改清單

### 修改文件（2 個）
- ✅ `src/pages/CategoryDetail.tsx`
  - 載入 courts
  - 添加 getCourtName helper
  - 小組賽顯示場地名稱
  - 對陣圖顯示場地名稱

- ✅ `src/pages/scorer/ScoringConsole.tsx`
  - 載入 courts
  - 添加 getCourtName helper
  - 場地信息顯示名稱

### 部署狀態
- ✅ 已構建
- ✅ 已部署
- ✅ https://courtside-25c9e.web.app

## 🎯 顯示效果

### 小組賽比賽列表
```
2024/12/21    Court 01  ← 顯示名稱
Alice vs Bob    21-19
已完成
```

### 對陣圖
```
Court 01      已完成  ← 顯示名稱
Alice    21  ✓
Bob      19
```

### 計分頁面
```
比賽狀態：進行中
場地：Court 01  ← 顯示名稱
```

---

## 🎊 完成

**場地顯示和分配時機說明**

- ✅ 所有頁面顯示場地名稱（不是 ID）
- ✅ 小組賽：發布時立即分配
- ✅ 第一輪：發布時立即分配
- ✅ 後續輪次：手動重新分配或等自動調度
- ✅ 智能分配策略（小組固定 + 決賽主場地）

**場地管理功能完整！** 🏟️✅

---

**修復日期**: 2024年12月21日  
**狀態**: ✅ 已完成並部署

