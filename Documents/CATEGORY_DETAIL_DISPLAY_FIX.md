...
// 3. 顯示每個小組
Object.keys(groups).sort().map((groupLabel) => (
  <Card>
    <h3>Group {groupLabel}</h3>
    {groups[groupLabel].map((match) => (
      <div>{match.player1Name} vs {match.player2Name}</div>
    ))}
  </Card>
))
```
...
1. **檢查 matches 數據**
   ...
   console.log("Group matches:", matches.filter(m => m.stage === "group"));
   console.log("Knockout matches:", matches.filter(m => m.stage === "knockout"));
   ```
