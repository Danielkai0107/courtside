# Sports 集合結構更新

## 更新日期
2024-12-23

## 概述
根據 Firestore 資料庫中的實際結構，更新了專案中 Sport 類型定義和相關功能，以支援更完整的運動項目配置系統。

## 新的 Sport 結構

### Firestore 文檔結構
```typescript
{
  id: string;                    // 文檔 ID（如 "badminton"）
  name: string;                  // 運動名稱（如 "羽毛球"）
  icon: string;                  // 圖示 emoji（如 ""）
  isActive: boolean;             // 是否啟用
  order: number;                 // 排序
  defaultPresetId: string;       // 預設規則 ID（如 "bwf_standard"）
  modes: string[];               // 可用模式（如 ["singles", "doubles"]）
  rulePresets: RulePreset[];     // 規則預設列表
  createdAt: Timestamp;
  updatedAt: Timestamp;
}
```

### RulePreset 結構
```typescript
{
  id: string;                    // 規則 ID（如 "bwf_standard"）
  label: string;                 // 顯示名稱（如 "BWF 標準賽制"）
  description: string;           // 說明（如 "每局21分，3局2勝，30分封頂"）
  config: {
    cap: number;                 // 參賽人數上限
    matchType: "set_based" | "point_based";  // 比賽類型
    maxSets: number;             // 最多局數
    pointsPerSet: number;        // 每局分數
    setsToWin: number;           // 獲勝所需局數
    winByTwo: boolean;           // 是否需要贏兩分
  }
}
```

## 主要變更

### 1. 型別定義更新
**檔案**: `src/types/index.ts`

- 移除了 `nameEn` 和 `availableFormats` 欄位
- 新增 `RulePresetConfig` 和 `RulePreset` 介面
- 更新 `Sport` 介面以符合新結構
- 新增 `modes` 和 `rulePresets` 欄位

### 2. 初始化腳本更新
**檔案**: `src/scripts/initSports.ts`

新增三個運動項目的初始化函數：

#### 羽毛球 (Badminton)
- **ID**: `badminton`
- **圖示**: 
- **規則預設**:
  1. BWF 標準賽制（每局21分，3局2勝，30分封頂）
  2. 單局30分制（快速比賽）
  3. 單局21分制（2分領先，30分封頂）

#### 匹克球 (Pickleball)
- **ID**: `pickleball`
- **圖示**: 🏓
- **規則預設**:
  1. 標準11分制（每局11分，3局2勝，15分封頂）
  2. 單局21分制（快速比賽）

#### 桌球 (Table Tennis)
- **ID**: `table_tennis`
- **圖示**: 🏓
- **規則預設**:
  1. 標準11分制（每局11分，5局3勝）
  2. 7局4勝制

### 3. CategoryManager 元件更新
**檔案**: `src/components/features/CategoryManager.tsx`

- 新增 `sport` prop 以接收運動項目資料
- 新增 `rulePresetId` 到 `CategoryFormData`
- 新增規則預設選擇器，顯示所有可用的規則預設
- 自動根據選擇的規則預設設定 `pointsPerSet`
- 新增 `handleRulePresetChange` 函數處理規則變更

### 4. CreateTournament 頁面更新
**檔案**: `src/pages/organizer/CreateTournament.tsx`

- 移除對 `nameEn` 的引用
- 使用 `sport.id` 作為 `sportType`
- 傳遞 `sport` prop 給 `CategoryManager`
- 在運動項目卡片中顯示 `modes` 而非 `nameEn`
- 從規則預設中自動獲取預設分數

## 使用方式

### 初始化運動項目資料

1. **在瀏覽器 Console 執行**:
```javascript
// 初始化所有運動項目
await window.initAllSports();

// 或單獨初始化
await window.initBadminton();
await window.initPickleball();
await window.initTableTennis();
```

2. **在程式碼中使用**:
```typescript
import { initAllSports } from './scripts/initSports';

// 在適當的地方呼叫
await initAllSports();
```

### 建立賽事時使用規則預設

當主辦方建立賽事並新增分類時：

1. 選擇運動項目（如羽毛球）
2. 進入分類設定步驟
3. 新增分類時，可以選擇預定義的規則預設
4. 選擇規則預設後，會自動填入對應的比賽配置

## 資料遷移

如果您的資料庫中已有舊的 Sport 文檔，需要執行以下步驟：

1. 備份現有資料
2. 執行 `initAllSports()` 創建新的運動項目
3. 更新現有 Tournament 的 `sportId` 引用
4. 刪除舊的 Sport 文檔（如果有）

## 優點

1. **靈活性**: 每個運動項目可以有多個規則預設
2. **可擴展性**: 輕鬆新增新的運動項目和規則
3. **一致性**: 所有賽事使用標準化的規則配置
4. **使用者友善**: 主辦方可以快速選擇常用的比賽規則
5. **維護性**: 規則集中管理，易於更新

## 未來擴展

1. 新增更多運動項目（排球、籃球等）
2. 支援自訂規則預設
3. 規則預設的版本控制
4. 匯入/匯出規則預設配置

## 測試檢查清單

- [ ] 執行 `initAllSports()` 成功創建運動項目
- [ ] 建立賽事時可以正確顯示運動項目
- [ ] 分類設定中可以選擇規則預設
- [ ] 選擇規則預設後正確填入分數配置
- [ ] 建立賽事流程完整無誤
- [ ] 檢查 Firestore 中的資料結構正確

## 相關檔案

- `src/types/index.ts` - 型別定義
- `src/scripts/initSports.ts` - 初始化腳本
- `src/services/sportService.ts` - 運動項目服務
- `src/components/features/CategoryManager.tsx` - 分類管理元件
- `src/pages/organizer/CreateTournament.tsx` - 建立賽事頁面

