...
1. **邀請時刻**：主辦方邀請已註冊紀錄員 → `inviteStaff` 創建 staff 記錄 + 創建通知（第1個通知）
2. **用戶刷新**：紀錄員刷新頁面 → `onAuthStateChanged` 觸發 → `linkShadowAccounts` 檢查到 `status === 'invited'` → **再次創建通知（第2個通知）**
3. **持續重複**：每次登錄或刷新都會重複步驟2
